...
### LongMemEval-S の結果（v0.18.5）

**これは未調整のベースラインであり、性能上限ではありません。** 継続利用では、モデルが能動的に重みを調整し、ユーザーの訂正や関連性フィードバックにも応じて更新できます。信頼できるフィードバックと継続的な調整による検索改善が期待されますが、その増分は未測定です。更新は Agent が明示的に行い、検索のたびに自動学習するわけではありません。

2026年9月13日、固定データセットを使い Apple M5 Pro Mac の4並列で2回実行しました。500/500問を処理し、470問を検索評価に使用。2回目は保存済みソースを再利用し、500問すべての検索順位が一致しました。

| 指標 | 1回目 | 2回目 |
| --- | ---: | ---: |
| 処理した問題数 | 500 / 500 | 500 / 500 |
| 検索評価の問題数 | 470 | 470 |
| 除外した回答辞退問題 | 30 | 30 |
...
| 平均遅延 | 509.883 ms | 679.759 ms |
| P50 | 507.697 ms | 665.454 ms |
| P90 | 663.061 ms | 931.238 ms |
| P95 | 731.766 ms | 990.375 ms |
| P99 | 888.555 ms | 1094.886 ms |

この結果には能動的な調整を含みません。アダプターは会話ソースを検索するだけで、モデルによる知識整理、関連性フィードバック、重み更新は行いません。実際の利用では、モデルが証拠を確認し、文書の重みやクエリ別フィードバックを明示的に調整して検索改善を図れます。毎回の検索で自動的に重みが変わるわけではなく、効果はフィードバックの品質に依存し、今回は未測定です。調整効果は未使用の評価問題で検証し、テストの正解を同じ評価に戻さないでください。これはローカル検索評価であり、公式ランキングや回答正解率ではありません。遅延は4並列負荷の値で、バージョン間の速度改善を証明する比較ではありません。

[データ: 1](../../benchmarks/results/2026-09-13-longmemeval-s-0185.json) · [データ: 2](../../benchmarks/results/2026-09-13-longmemeval-s-0185-second.json) · [LongMemEval-S](../../benchmarks/agent_memory/README.md#longmemeval-s)

...
fn validate_tutor_schema(connection: &Connection) -> Result<()> {
    for (table, columns) in [
        ...
        (
            "soul_settings",
            &["singleton", "max_bytes", "revision", "updated_at"],
        ),
        (
            "soul_settings_history",
            &["revision", "max_bytes", "changed_at"],
        ),
        (
            "soul_proposals",
            &[
        ...
        (
            ...
            &["fact_id", "revision", "snapshot_json", "changed_at"],
        ),
        (
            "tutor_goals",
            &[
        ...
        (
            "tutor_goal_criteria",
            &["id", "goal_id", "ordinal", "description"],
        ),
        (
            "tutor_goal_evidence",
            &[
        ...
        (
            "tutor_goal_history",
            &["goal_id", "revision", "snapshot_json", "changed_at"],
        ),
        (
            "tutor_plans",
            &[
...
fn takeover_turn(store: &mut Store, input: TurnTakeover) -> Result<Value> {
    ...
    if changed != 1 {
        ...
        return Err(Error::new("revision_conflict", "turn revision changed"));
    }
    tx.execute(

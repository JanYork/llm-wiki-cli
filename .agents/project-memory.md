...
- `v0.17.12` is a release candidate and has not been published. Portable-memory-archive Plan `a11c321150c2abb866d2f24189dde635` completed at revision 8, and CodeGraph MCP/update Plan `a364893cda03a3c7b98f15cdd28dda76` completed at revision 13. Release Plan `0c6eb7dc931174ffdaf9ac9b94e83b86` is active at revision 1; its current step is exact staging/commit followed by a clean Pro package. The candidate adds exact-scope plaintext `compress`/`decompress`/bounded resumable `merge`, confirmation-token-gated overwrite, receipt-resumed derived rebuild, native read-only `lwc_codegraph` raw passthrough with a 60-second outer deadline over CodeGraph's 45-second queue, exact-symbol routing, and silent one-hour-throttled one-shot update readiness.
